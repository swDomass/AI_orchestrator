...
from config import TOOL_MAX_ITERATIONS, TOOL_REVIEW_TIMEOUT_SEC, TOOL_FIX_TIMEOUT_SEC
...
def _parse_findings(text: str) -> list[str]:
    ...
    return [line.strip() for line in text.splitlines() if FINDING_RE.match(line)]


_REVIEW_PROMPT = """Perform a code review of the current working directory.
...
_FIX_PROMPT = """You are fixing issues found by a code review (iteration {iteration}).
...
class ReviewLoopTool(BaseTool):
    ...
    def run(
        ...
        for iteration in range(1, TOOL_MAX_ITERATIONS + 1):
            ...
            findings = _parse_findings(review_result.output)
