...
| 53 | Per-task auto-commit — a successful run commits its own paths to `orch/<slug>-<date>` and restores them in the working tree | DONE (2026-09-11). HEAD is never moved (temp index + `commit-tree` + `update-ref`). Two rules gate it: the index must agree with HEAD, and the path must have been clean when the run started. Default ON, `GIT_AUTO_COMMIT=false` / `#no-commit` to opt out. Not built (KUER): retention for old `orch/*` branches, `#parallel`/`#worktree`, push/PR/merge, model-written commit messages. |
...
| `tests/conftest.py`'s `_guard_real_vault_and_docs_untouched` is rarely flaky against a live orchestrator (P2-2, oc r1, 2026-09-16) | A `run_orchestrator.ps1 --watch` process running against the same real vault during a local test run can complete a task inside the suite's ~2-minute session window, writing/moving a real file the same way a leaking test would — the guard cannot tell the two apart from the file system alone, so it fails the run either way (kept hard on purpose: a false alarm here is cheaper than a missed leak). The failure message now names both candidate causes and lists the concretely new/missing/hash-changed paths with mtime; recognizable by that message, and resolved by re-running the suite with the live orchestrator idle. |
| `usage-suggest` heartbeat "timeouts" (investigated 2026-09-16, **not a defect**) | The 2026-09-16 Betriebsprüfung reported "26 of 52 usage-suggest heartbeat calls since 2026-09-02 timed out" as if it were a technical failure. Traced to source: `usage_suggester.check_and_suggest()` (`usage_suggester.py:140-170`) sends the Telegram suggestion list and then does `event.wait(timeout=USAGE_SUGGEST_TIMEOUT_SEC)` (5 min, `config.py:780`) for a `/pick N`/`/decline` reply; on expiry it logs `"usage-suggest: timeout, no response"` (line 169) — exactly the module's own documented behaviour ("If no response arrives within 5 minutes the suggestions expire silently", module docstring). Every one of the 26 `[usage_suggester]` INFO lines in `logs/orchestrator.log` since 2026-09-02 is this exact string, verbatim, verified by a full scan (`grep -c` plus a Python pass classifying every `[usage_suggester]` line by its message prefix: 26/26 are `timeout`, 0 are `declined`/`picked`/`error`). It is a **user-response timeout**, not an LLM/network/process failure: `heartbeat._check_usage_suggest()` (`heartbeat.py:401-428`) already launches the check in a daemon thread and returns immediately, so a heartbeat *poll* cannot itself hang or time out on this handler. The reported denominator (52) could not be reconstructed from the available logs — `usage-suggest` has no persistent invocation counter (`heartbeat-state.json` only tracks items with interval ≥ 1 day, `heartbeat.py` design note #4); only the numerator (26 completed "timeout" outcomes) is directly evidenced. **No fix built**: there is no code defect here to correct without changing product behaviour (response window length, suggestion timing, or whether/how a missed suggestion is followed up) — that is a scope decision for the user, not a bug. If the underlying complaint is "suggestions are consistently ignored," that is worth a deliberate ask, not a speculative code change. |
| `#parallel` subtasks set no provider cooldown for ANY error code, and the `#parallel` PARENT never retries/parks at all (pre-existing, named 2026-09-16 Runde 2/3 while extending `auth_expired`) | Two separate, both pre-existing facts, checked in Runde 3 for an `auth_expired`-specific version of the P1 finalization trap and found to be the SAME behaviour every error code already gets, not a new gap: (1) `parallel_runner._run_single_subtask()` never calls `provider.set_cooldown()` — not for `rate_limit`, not for `unreachable`, and now not for `auth_expired` either (which DID get the one-time Telegram notice via the shared `_notify_auth_expired_once()` dedup, same helper as the non-parallel paths, no second mechanism). (2) `run_once()`'s `#parallel` handling (`orchestrator.py`, `#parallel` branch) finalizes the PARENT queue line unconditionally right after `run_parallel()` returns — `_finalize_task_with_result_checked(..., failed=not success_all)` — regardless of which error code caused a subtask to fail. There is no retry/park mechanism for `#parallel` parents at all (by design — see Design decisions #7: "the parent gets one aggregated result"), so a subtask failing on `auth_expired` is finalized ❌ exactly as terminally as one failing on `rate_limit` today. The P1 fix in `_execute_tool_task()` (below) does not change this: `_run_single_subtask()` never reads `outcome.finalized`/`outcome.retryable`, only `outcome.success`/`.error_code`/`.output`, so the override is invisible to the parallel path either way. Not fixed here: closing either half needs a design decision (cooldown: how a cooldown set from inside one parallel worker thread should interact with sibling threads racing the same provider singleton; retry: whether/how a `#parallel` parent could ever be re-parked per-subtask instead of as one unit) — out of scope for an `auth_expired` classification fix. |
| Tools that wrap EVERY provider failure into their own exception text lose ALL provider error codes, not just `auth_expired` (pre-existing, named 2026-09-16 Runde 4) | `tools/scientific_investigation_phases.py` (phases 0.5-7) raises `RuntimeError(f"... failed: {result.error}")` on any provider failure; `tools/scientific_investigation.py:429-435` catches that and reclassifies it as `error_code="phaseN_failed", retryable=False` — the ORIGINAL provider code (`rate_limit`, `unreachable`, `auth_expired`, …) never survives as a `error_code_of()`-extractable head, only as free text inside the wrapped message, if at all. `auth_expired` is recovered from that text since this round via `providers.base.contains_auth_expired()` in `_execute_tool_task()` (a word-bounded token scan, not a second classification source — see the P1 fix above), so THAT one code is now safe for every tool that does this, including future ones. `rate_limit`/`unreachable`/`timeout`/etc. are not: a phase that hits a rate limit still finalizes the task permanently instead of parking it for a quota reset. Not fixed — `scientific_investigation` runs over the queue in practice essentially never (see Memory/ROADMAP: the tool path is de facto `dev-loop`), so rebuilding its eight phase functions' error handling to preserve every provider code would be a disproportionate change for this auftrag's scope. Revisit if the tool ever sees real queue traffic. |
