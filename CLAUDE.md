...
- **`queue_manager.py`** — Obsidian MD queue parser, sidecar `.lock`, regex metadata, `#needs:`/`#id:` two-pass deps, subtask-aware mutations; P1 `#worktree`/`#keep-worktree` tags for parallel isolation; `#verify:<script>` post-task outcome check (unquoted path bounded by whitespace AND `#` — without the `#` bound adjoining tag text lands inside the script path, the lookup fails forever, and fail-closed turns that into a permanent alarm on every *successful* run); `collect_file_context()` returns context blocks WITHOUT the task text so `_build_prompt` can place the instruction last (see below). **Known gap:** `MODEL_TAG_RE` hard-codes 6 of the 20 model aliases — the other 14 route to the right provider but never force the model (details + fix vector in `components.md`)
