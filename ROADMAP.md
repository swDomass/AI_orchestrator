...
A seventh was closed on 2026-09-17: **tools that wrapped every provider
failure into their own exception text no longer lose the original error
code.** `providers/base.py` gained `ProviderCallError(RuntimeError)`, carrying
the raw `RunResult.error` as `.provider_error`; all 7 provider-failure raise
sites across `tools/scientific_investigation_phases.py` (Phase 0, 0.5),
`_phase2.py` (`_run_persona_call`), `_phase4.py` (Phase 4 synthesis),
`_phase5.py` (Phase 5b), and `_phase7.py` (review + rework, 2 sites) now raise
`ProviderCallError` instead of a bare `RuntimeError`. `scientific_investigation.py`
catches it before the pre-existing generic `except (RuntimeError, ValueError)`
in the 5 affected `try` blocks (Phase 0, 0.5, 2, 4, 7) and classifies via
`error_code_of()`/`is_transient()` — no separate taxonomy list, per the
existing rule. A rate-limited/unreachable/timed-out phase is now requeued
instead of finalized as a permanent failure; the `auth_expired` behaviour
(previously recovered only via the `contains_auth_expired()` text-scan
workaround) is unchanged, now reached directly instead of through that
workaround. Tests per error class (transient, terminal/unclassifiable,
