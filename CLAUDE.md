...
- **`dispatcher.py`**: Provider selection with fallback chain (Claude → Gemini → Codex), cooldown management, provider-bound model aliases for all three CLIs: Claude (`#claude_haiku`, `#claude_sonnet`, `#claude_opus`), Gemini (`#gemini_pro`, `#gemini_flash`), Codex (`#codex_mini`). `get_provider_by_name()` for cross-provider tool access
...
- **`providers/claude.py`**: Uses `--output-format json` to capture actual token usage. `_parse_json_response()` extracts `result` text + `usage.input_tokens`/`output_tokens` from Claude CLI JSON output. Passes `--model <id>` when `_forced_model` is set
...
- **`tools/base_tool.py`**: `BaseTool` ABC + `_build_system_prompt(provider_name, memory_context, tool_name, cwd)` assembles system prompt with 4 memory layers (curated, daily, lessons, task context). All tool subclasses pass `tool_name=self.name, cwd=cwd` for CWD-filtered lesson injection
